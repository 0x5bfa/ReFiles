...
public sealed class ItemFeatureCombiningTests
{
	...
	public async Task ThumbnailCompositionUsesPriorityAndStopsAfterSuccess()
	{
		...
		var source = combiner.Combine(context, [ new ItemFeatureOption<IThumbnailSource>(lower, 10, "lower", ItemFeatureLifetime.Shared), new ItemFeatureOption<IThumbnailSource>(higher, 20, "higher", ItemFeatureLifetime.Shared), ])!;
	...
	public async Task ThumbnailCompositionFallsBackWhenHigherPriorityReturnsNull()
	{
		...
		var source = new ThumbnailSourceCombiner().Combine(context, [ new ItemFeatureOption<IThumbnailSource>(first, 20, "first", ItemFeatureLifetime.Shared), new ItemFeatureOption<IThumbnailSource>(second, 10, "second", ItemFeatureLifetime.Shared), ])!;
	...
	public async Task PreviewCompositionRoutesByPriority()
	{
		...
		var source = new PreviewSourceCombiner().Combine(context, [ new ItemFeatureOption<IPreviewSource>(first, 5, "first", ItemFeatureLifetime.Shared), new ItemFeatureOption<IPreviewSource>(second, 1, "second", ItemFeatureLifetime.Shared), ])!;
	...
	public async Task PreviewCompositionStopsFallbackAfterBlockedResult()
	{
		...
		var source = new PreviewSourceCombiner().Combine(context, [ new ItemFeatureOption<IPreviewSource>(blocked, 20, "blocked", ItemFeatureLifetime.Shared), new ItemFeatureOption<IPreviewSource>(fallback, 10, "fallback", ItemFeatureLifetime.Shared), ])!;
	...
	public async Task PropertyCompositionMergesSourcesWithHigherPriorityWinning()
	{
		...
		var source = new PropertySourceCombiner().Combine(context, [ new ItemFeatureOption<IPropertySource>(low, 10, "low", ItemFeatureLifetime.Shared), new ItemFeatureOption<IPropertySource>(high, 20, "high", ItemFeatureLifetime.Shared), ])!;
