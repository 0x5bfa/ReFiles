...
詳細ビューの列幅、レイアウトモード、並べ替えとグループ化の選択、項目サイズは、参照場所の表示方法を表します。
...
- 並べ替えプロパティ ID と方向。
- グループ化プロパティ ID と方向。プロパティ ID が `null` の場合はグループ化しません。
- 任意の項目サイズ。
...
セッションは新しいコンテキストの最初の bounded batch が利用可能になった時点で、generation、アクティブなコンテキスト、項目投影を暫定的に公開します。
以前のコンテキスト、投影、選択、表示値は navigation の確定まで rollback snapshot として保持します。
ナビゲーションが失敗またはキャンセルされた場合は、公開済みを含む新しいコンテキストと部分的な項目を破棄し、以前の状態を 1 回の reset で復元します。
成功時だけ以前のコンテキストと項目を非同期に破棄します。
...
投影は `name` と `System.ItemNameDisplay` を `IStorableModel.Name` から直接並べ替えます。フォルダーは昇順と降順のどちらでもファイルより前に置きます。その他のプロパティ ID には、`BrowseItemPresentation` にすでに公開されている値を使います。
利用できない値はどちらの方向でも最後に置き、名前と安定した識別情報を決定論的なタイブレーカーにします。

WinUI の ViewModel は `CollectionViewSource` を公開します。グループ化しない場合は既存の増分コレクションを直接ソースにし、グループ化する場合だけ UI 用のグループ投影を作ります。
名前、更新日、作成日、サイズ、種類のグループ化をサポートし、後から届く表示プロパティによる再グループ化は低優先度でまとめます。Core の項目投影や安定した選択キーはグループ化のために複製しません。
...
プロパティに基づく並べ替えは、要求された値が届くたびに再評価します。表示列と並べ替えプロパティに加え、現在のグループ化プロパティも先読み対象にします。このストアはスナップショット単位で、モデルの置換時にクリアまたは無効化されます。
